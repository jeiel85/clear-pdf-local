# ClearPDF Local — *open-local-scan*

> **open-local-scan — 유료 스캐너(vFlat 등)의 프리미엄 기능을, 오픈소스로 그리고 100% 오프라인으로.**
>
> AI 곡면 보정·자동 스캔·온디바이스 OCR까지, 유료 앱에서나 보던 문서 스캔 기능을 **인터넷 권한 없이** 기기 안에서만 처리하는 오픈소스 Android 문서 스캐너 & PDF 도구입니다.

**ClearPDF Local** (a.k.a. **open-local-scan**) is a free, open-source, fully-offline document scanner and PDF toolkit for Android — a privacy-first alternative to premium scanners like vFlat. AI curved-page dewarping, auto-scan, and on-device OCR all run on-device with **zero network requests** — the app never even declares the `INTERNET` permission. Built with Kotlin and Jetpack Compose.
...
- **AI 곡면 보정 / 책 펴기 (Neural Dewarping)**: 펼친 책처럼 휘어진 페이지를 찍어도, 스캔한 페이지의 'Flatten' 메뉴로 굽은 글줄과 책등 곡률을 평평하게 펴냅니다. UVDoc 모델을 ONNX Runtime으로 **온디바이스 실행**(중급 태블릿에서 ~0.3초)하며, 모델이 앱에 번들되어 인터넷 없이 동작합니다. **'자동 펴기(Auto-flatten)'** 를 켜면 자동 스캔으로 찍히는 페이지가 즉시 자동으로 펴져, **책을 넘기기만 하면 평면화된 스캔이 차곡차곡 쌓입니다.**
- **손가락 제거 (Finger Removal, best-effort)**: 책을 잡은 손가락이 페이지 가장자리에 함께 찍혔을 때, 스캔 페이지의 'Remove finger'로 스킨 색상 검출 + 인페인팅(OpenCV)으로 지웁니다. 가장자리에서 들어온 손가락에 한해 동작하는 *베스트 에포트* 기능이며, 손가락이 가렸던 글자까지 복원하지는 못합니다.
- **반사 줄이기 (Glare Reduction, best-effort)**: 광택지의 작은 빛 반사를 'Reduce glare'로 줄입니다. 흰 종이 배경과 구분하기 위해 탑햇(top-hat)으로 *국소 배경보다 밝은 작은 반사 점*만 골라 인페인팅하며, 크게 날아간 포화 영역은 복원하지 못합니다.
